"""Ray-free resident hands.detect adapter with revision-owned mask behavior.
...
* The detector (Ultralytics YOLO ``detector.pt``) produces boxes/scores/sides.
  Legacy revisions prompt SAM2 for real masks; ``hands-yolo-v2`` does not load or
  call SAM2 and never fabricates a mask.
...
* One canonical HxW compatibility bucket. Incompatible/overweight items are
  ...
  (``batch=`` passed to Ultralytics for a single fused forward).
* YOLO-only detections have visibility 1.0 and uncertainty from detector score
  plus box-edge proximity; empty images remain empty.
...
HANDS_YOLO_V2_REVISION = "hands-yolo-v2"


class HandsBackend(Protocol):
    """The detector boundary shared by all hands revisions.

    ``detect`` receives contiguous uint8 HWC images and returns one mapping per
    image with ``boxes`` [K,4] xyxy, ``scores`` [K], and ``sides`` [K]
    (0=left,1=right). Legacy SAM2 backends additionally expose ``mask``.
    """
...
class HandsModelConfig:
    ...
    @property
    def uses_sam2(self) -> bool:
        return self.model_revision != HANDS_YOLO_V2_REVISION
...
def _load_hands_backend(config: HandsModelConfig) -> HandsBackend:
    """Load the resident YOLO detector and SAM2 only for mask-producing revisions.

    ``hands-yolo-v2`` imports and instantiates no SAM2 objects, so it avoids both
    the SAM2 checkpoint residency cost and all image/prompt prediction calls.
    """
    import numpy as np
    ...
    class TorchHandsBackend:
        ...
        def mask(self, image_rgb: Any, boxes: Any) -> Any:
            if sam2_predictor is None:
                raise RuntimeError("SAM2 masks are unavailable for hands-yolo-v2")
...
def _compute_visibility_uncertainty(masks: Any, boxes: Any, scores: Any, height: int, width: int) -> tuple[Any, Any]:
    """Compute legacy SAM2-mask visibility with shared score/edge uncertainty."""
    import numpy as np

    k = len(scores)
    visibility = np.zeros((k,), np.float32)
    uncertainty = _compute_yolo_visibility_uncertainty(boxes, scores, height, width)[1]
    for i in range(k):
        x1, y1, x2, y2 = boxes[i]
        ...
        mask_area = float(masks[i][y1c:y2c, x1c:x2c].sum()) if (y2c > y1c and x2c > x1c) else 0.0
        visibility[i] = float(np.clip(mask_area / box_area, 0.0, 1.0))
    return visibility, uncertainty


def _compute_yolo_visibility_uncertainty(boxes: Any, scores: Any, height: int, width: int) -> tuple[Any, Any]:
    """Compute the YOLO-only contract: detected hands are fully visible."""
    import numpy as np

    k = len(scores)
    visibility = np.ones((k,), np.float32)
    uncertainty = np.zeros((k,), np.float32)
    for i, box in enumerate(boxes):
        x1, y1, x2, y2 = box
        edge_penalty = 0.1 if x1 <= 1 or y1 <= 1 or x2 >= width - 1 or y2 >= height - 1 else 0.0
        uncertainty[i] = float(np.clip((1.0 - float(scores[i])) + edge_penalty, 0.0, 1.0))
    return visibility, uncertainty
...
class HandsAdapter:
    ...
    async def infer_batch(self, requests: Sequence[_PreparedHandsRequest]) -> list[HandsDetectResponse]:
        ...
        Legacy revisions generate real SAM2 masks post-forward. ``hands-yolo-v2``
        takes the detector-only branch: no SAM2 invocation, no mask payload, and
        ``sam2_mask_calls == 0``.
        """
        ...
        try:
            ...
            for index, det in enumerate(detections):
                ...
                k = len(scores)
                if k == 0:
                    per_image.append(HandDetection.empty(height, width, include_masks=self._config.uses_sam2))
                    continue
                if self._config.uses_sam2:
                    mask = getattr(self._backend, "mask", None)
                    if not callable(mask):
                        raise RuntimeError("SAM2 revision backend does not provide mask generation")
                    masks = await asyncio.to_thread(mask, images[index], boxes)
                    sam2_calls += 1
                    visibility, uncertainty = _compute_visibility_uncertainty(masks, boxes, scores, height, width)
                    mask_payload = _as_tensor_payload(masks.astype(np.uint8))
                else:
                    visibility, uncertainty = _compute_yolo_visibility_uncertainty(boxes, scores, height, width)
                    mask_payload = None
                per_image.append(
                    HandDetection(
