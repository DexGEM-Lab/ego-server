...
DROID_MAX_SESSIONS_PER_REPLICA = 8
...
class LaneDispatcher:
    ...
    def _forward(self, replica_url: str, path: str, body: bytes, headers: Iterable[tuple[str, str]]) -> UpstreamResponse:
        parsed = urlsplit(replica_url)
        if parsed.scheme != "http" or not parsed.hostname:
            raise OSError(f"invalid HTTP replica URL: {replica_url}")
        connection = http.client.HTTPConnection(parsed.hostname, parsed.port or 80, timeout=self.backend_timeout_s)
        try:
...
class _DispatcherHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        allowed = (
            clean_path.startswith("/droid.") if self.server.service_kind == "droid"
...
__all__ = [
    "BACKEND_TIMEOUT_S", "DROID_MAX_SESSIONS_PER_REPLICA", "DROID_REPLICAS", "DispatcherHTTPServer",
