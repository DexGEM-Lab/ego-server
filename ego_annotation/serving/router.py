...
``28000 + gpu_id`` (GPU0=28000 ... GPU4=28004, GPU6=28006). This is the canonical
...
# 28000 + gpu_id (GPU0=28000, GPU1=28001, GPU2=28002, GPU3=28003, GPU4=28004, GPU6=28006). These
...
DEFAULT_MODEL_REVISIONS: dict[ModelApiName, str] = {
    ...
    ModelApiName.HANDS_DETECT: "hands-yolo-v2",
...
MODEL_SERVICES: tuple[ModelService, ...] = (
    ...
    ModelService(name="wilor", gpu_id=4, api_names=(ModelApiName.WILOR_RECONSTRUCT,)),
...
def _default_serve_http_port(gpu_id: int) -> int:
    ...
    GPU1=28001, GPU2=28002, GPU3=28003, GPU4=28004, GPU6=28006. Distinct from Ray lifecycle
...
def canonical_cluster_endpoints() -> tuple[ClusterEndpointConfig, ...]:
    """The six committed GPU groups (0/1/2/3/4/6) and their Serve HTTP ports."""
    return tuple(_cluster_endpoint_for_group(gpu) for gpu in (0, 1, 2, 3, 4, 6))
