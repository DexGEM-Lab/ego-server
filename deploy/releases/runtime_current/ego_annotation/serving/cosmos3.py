...
class Cosmos3Adapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._batch_snapshot = BatchSnapshotTracker(
            service="cosmos3.reason", replica_id=config.replica_id,
            capacity={"kind": "vllm_continuous_batch_request", "max_batch_size": None, "max_ongoing_requests": 16},
        )
    ...
    async def infer_one(self, prepared: _PreparedRequest) -> Cosmos3Response:
        ...
        execution = self._batch_snapshot.begin(ExecutionSpec(
            operation="reason.generate", execution_kind="vllm_continuous_batch_request", request_count=1,
            item_count=len(prepared.request.media), item_kind="media_item", image_item_count=None,
            forward_count=1, max_batch_size=None,
        ))
        try:
        ...
        if final_output is None:
            execution.finish(success=False, error_type="NoOutput")
            return Cosmos3Response(
                ownership=prepared.request.ownership,
                error=ServiceError(
                    ErrorCode.MODEL_FAILURE,
