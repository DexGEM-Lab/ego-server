...
def add_camera_and_hybrid_outputs(run_root: Path, *, camera_backend: str | None = None) -> None:
    ...
    if camera_backend is not None:
        stage_payload["camera_backend"] = camera_backend
        stage_payload["backend"] = camera_backend
        stage_payload["gauge_declaration"]["trajectory_frame"] = "VGGT/Omega local anchor world gauge"
    write_json(cam_dir / "v22_camera_trajectory_stage.json", stage_payload)
    write_json(
...
def test_adapter_promotes_camera_and_hybrid_rows_when_stages_exist(tmp_path: Path) -> None:
    ...
    cameras = ndjson(Path(manifest["tables"]["head_camera"]["ndjson"]))
    assert {row["source"] for row in cameras} == {"droid_full_frame_video_tracking"}
    errors = ndjson(Path(manifest["events"]["errors"]["ndjson"]))
    codes = {row["code"] for row in errors}
...
def test_adapter_preserves_vggt_camera_backend_provenance(tmp_path: Path) -> None:
    run_root = make_run_root(tmp_path)
    add_camera_and_hybrid_outputs(run_root, camera_backend="vggt_camera_contract_backend")
    args = parse_args(["--run-root", str(run_root), "--output-root", str(tmp_path / "bundles"), "--job-id", "with_vggt_camera"])
    manifest_path = adapt(args)
    manifest = read_json(manifest_path)

    cameras = ndjson(Path(manifest["tables"]["head_camera"]["ndjson"]))
    assert {row["source"] for row in cameras} == {"vggt_camera_contract_backend_video_tracking"}
    assert {row["gauge"] for row in cameras} == {"VGGT/Omega local anchor world gauge"}
    errors = ndjson(Path(manifest["events"]["errors"]["ndjson"]))
    camera_warning = next(row for row in errors if row["code"] == "head_camera_video_derived_uncertain_gauge")
    assert "D4 camera trajectory stage" in camera_warning["mechanism"]
    assert "DROID provides" not in camera_warning["mechanism"]
