...
def test_captioning_stage_uses_cosmos_semantic_review(tmp_path: Path) -> None:
    run_root = make_run_root(tmp_path)
    review = run_root / "state/semantic_clips/v22_cosmos_semantic_review.json"
    write_json(
        review,
        {
            ...
            "semantic_rows": [
                {
                    "start_frame": 0,
                    "end_frame": 2,
                    "caption": "left hand operates a mop",
                    "per_hand": {
                        "left": {"action_state": "operating_object", "object": "mop", "object_rigidity": "rigid handle", "object_assembly": True, "contact_location": "handle"},
                        "right": {"action_state": "no_action", "object": "none", "object_rigidity": "not_applicable", "object_assembly": False, "contact_location": "none"},
                    },
                }
            ],
        },
    )
    result = build_captions(parse_caption_args(["--run-root", str(run_root)]))
    assert result["status"] == "ok"
    assert result["source_kind"] == "semantic_review_json"
    assert result["summary"]["timeline_coverage_fraction"] == 1.0
    row = result["semantic_rows"][0]
    assert row["per_hand"]["left"]["action_state"] == "operating_object"
    assert row["per_hand"]["right"]["action_state"] == "no_action"


def test_captioning_stage_does_not_default_to_agent_review(tmp_path: Path) -> None:
    run_root = make_run_root(tmp_path)
    review = run_root / "state/semantic_clips/v22_semantic_agent_review.json"
    write_json(review, {"schema": "legacy_agent_review", "semantic_rows": [{"start_frame": 0, "end_frame": 2, "caption": "legacy agent row"}]})
    result = build_captions(parse_caption_args(["--run-root", str(run_root)]))
    assert result["status"] == "source_absent_no_caption_rows"
