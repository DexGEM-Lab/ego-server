...
from v20_common import ContractError, ensure_no_gt_in_prediction, load_json, read_depth_image_m, write_json
...
def read_mask(path: Path | None, shape_hw: tuple[int, int]) -> np.ndarray | None:
    ...
    if mask is None:
        raise ContractError(f"could_not_read_sam2_mask: {path}")
    if mask.shape != shape_hw:
        mask = cv2.resize(mask, (shape_hw[1], shape_hw[0]), interpolation=cv2.INTER_NEAREST)
    return mask > 0


def unproject(mask: np.ndarray, depth_m: np.ndarray, intr: np.ndarray, max_points: int) -> np.ndarray:
...
def load_sam2_tracks(root: Path | None, objects: list[dict[str, Any]]) -> dict[str, dict[int, dict[str, Any]]]:
    ...
    for obj in objects:
        ...
        candidates = [
            root / track_id / "sam2" / "sam2_track.json",
            root / object_id.replace(":", "_") / "sam2" / "sam2_track.json",
        ]
...
def load_target_object_plan(path: Path | None, public_roster: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    if path is None:
        raise ContractError("missing_v20_target_object_plan: public_object_model_roster_is_not_a_prediction_target_list")
    payload = load_json(path)
...
def hand_rows_from_mano(blob: dict[str, Any] | None, frame_idx: int, intr: np.ndarray) -> list[dict[str, Any]]:
    if blob is None:
        return []
    ...
    for row_idx in rows.tolist():
        ...
        uv = np.full((joints.shape[0], 2), np.nan, dtype=float)
        z = joints[:, 2]
        valid_z = np.isfinite(joints).all(axis=1) & (z > 1.0e-6)
        fx, fy, cx, cy = intr.astype(float)
        uv[valid_z, 0] = fx * joints[valid_z, 0] / z[valid_z] + cx
        uv[valid_z, 1] = fy * joints[valid_z, 1] / z[valid_z] + cy
        hands.append(
            {
                "hand_id": f"hand:{side}",
                "hand_side": side,
                ...
                "metric_mano_state": {
                    ...
                    "hand_side": side,
                    "coordinate_status": "dexycb_color_camera_opencv_positive_z_prediction_side_mano",
...
def object_row(
    ...
    intr: np.ndarray,
    points: np.ndarray,
    geometry_fit: dict[str, Any] | None,
    require_masks: bool,
) -> dict[str, Any]:
    object_id = str(obj["object_id"])
    visible = bool(track and track.get("visible") and mask is not None and np.any(mask))
    if require_masks and not visible:
        raise ContractError(f"missing_visible_prediction_mask_for_object_frame: {object_id}:{frame_idx}")
    surface = unproject(mask, depth_m, intr, 512) if visible and mask is not None else np.zeros((0, 3), dtype=np.float32)
    ...
    geom = {
        "source": "sam2_mask_native_rgbd_unprojection" if visible else "missing_prediction_visible_surface",
        "object_id": object_id,
        ...
        "bbox_xyxy": bbox,
        "center_xy": center_xy,
        "coordinate_status": "dexycb_color_camera_as_world_identity_for_benchmark_prediction",
        "claim_scope": "visible object surface from prediction-side SAM2 mask and native depth; not complete object pose by itself",
    }
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if manifest.get("dataset") != "dexycb":
        raise ContractError("build_v20_dexycb_base_annotations_requires_dexycb_manifest")
    ...
    points_by_object = {str(obj["object_id"]): load_points(Path(obj["points_path"]), args.max_model_points) for obj in objects}
    sam2_tracks = load_sam2_tracks(args.sam2_root, objects)
    ...
    for row in manifest.get("frames", []):
        ...
        for obj in objects:
            object_id = str(obj["object_id"])
            track = sam2_tracks.get(object_id, {}).get(frame_idx)
            ...
            obj_row = object_row(obj, frame_idx, track, mask, depth_m, intr, points_by_object[object_id], geometry_fit_reports.get(object_id), args.require_sam2_masks)
            if obj_row["visible"]:
                object_visible_count += 1
            objs.append(obj_row)
        hands = hand_rows_from_mano(mano, frame_idx, intr)
        ...
        frames_out.append(
            {
                ...
                "factor_graph_solution": {
                    "variables": {
                        "hand_wrist": [
                            {
                                "variable_id": f"hand_wrist::{hand['hand_side']}",
                                "estimate": hand["metric_mano_state"]["joints_current_v18_world_m"][0],
                                "source": "prediction_side_metric_mano",
                            }
                            for hand in hands
                        ],
                        "object_se3": [
                    ...
                    "solution": {
                        "status": "prediction_side_base_state_before_v20_refinement",
                        "active_contact_hypotheses": 0,
                        "unresolved_or_contradicted_contact_hypotheses": 0,
                    },
                },
            }
        )
    ...
    annotations = {
        "schema": "v20_dexycb_prediction_base_annotations.v0",
        "case_id": args.case_id,
        "mode": "v20_benchmark",
        "benchmark_mode_detail": "prediction_eval_refs_sealed",
        "claim_scope": "Prediction-side DexYCB base annotations from public RGB-D/CAD inputs and model outputs only. Eval refs are withheld until evaluator.",
        ...
        "target_selection_policy": "Only object_plan targets are rendered/optimized. Dataset public object roster is a model library, not a target list.",
        "frames": frames_out,
        "module_counts": {
            "frames": len(frames_out),
            "objects_per_frame": len(objects),
            "visible_object_rows": int(object_visible_count),
            "hand_rows": int(hand_count),
            "missing_mask_rows": int(len(missing_masks)),
        },
        "limitations": {
            "hand_state": "requires prediction-side MANO npz; absent if hand_rows=0",
            "object_pose": "weak visible-depth centroid unless later V20 geometry promotion/pose solve improves it",
            "eval_refs": "not loaded by this builder",
        },
        "object_plan_payload": object_plan_payload,
    }
    ensure_no_gt_in_prediction(annotations, "v20_dexycb_base_annotations")
    write_json(args.output_annotations, annotations)
    report = {
        "status": "ok",
        "method": "build_v20_dexycb_base_annotations",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build prediction-side V20 DexYCB base annotations from RGB-D/CAD/model outputs only.")
    parser.add_argument("--dataset-manifest", type=Path, required=True)
    parser.add_argument("--input-manifest", type=Path)
    parser.add_argument("--object-plan", type=Path, required=True)
    parser.add_argument("--sam2-root", type=Path)
    ...
    parser.add_argument("--require-sam2-masks", action="store_true")
