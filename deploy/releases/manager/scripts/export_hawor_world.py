...
from scripts.adapt_droid_to_hawor import adapt_droid_to_hawor
...
def write_motion_preparation_report(
    *,
    output_dir: Path,
    video_path: Path,
    ...
    imgfiles: list[Path],
    img_focal: float,
    focal_cache_contract: dict,
) -> dict:
    if not imgfiles:
        raise RuntimeError("HaWoR motion preparation produced no extracted images")
    tracks_dir = seq_folder / f"tracks_{int(start_idx)}_{int(end_idx)}"
    mask_path = tracks_dir / "model_masks.npy"
    chunks_path = tracks_dir / "frame_chunks_all.npy"
    if not mask_path.is_file():
        raise RuntimeError(f"HaWoR motion preparation did not produce model masks: {mask_path}")
    if not chunks_path.is_file():
        raise RuntimeError(f"HaWoR motion preparation did not produce frame chunks: {chunks_path}")
    masks = np.load(mask_path, mmap_mode="r", allow_pickle=False)
    if masks.ndim != 3 or masks.shape[0] != len(imgfiles):
        raise RuntimeError(
            f"HaWoR model masks do not cover the prepared timeline: shape={getattr(masks, 'shape', None)} frames={len(imgfiles)}"
        )
    if masks.dtype.kind not in {"b", "u", "i", "f"}:
        raise RuntimeError(f"HaWoR model masks have unsupported dtype: {masks.dtype}")
    mask_min = float("inf")
    mask_max = float("-inf")
    for frame_idx in range(int(masks.shape[0])):
        frame = np.asarray(masks[frame_idx], dtype=np.float32)
        if not np.isfinite(frame).all():
            raise RuntimeError(f"HaWoR model masks contain non-finite values at frame {frame_idx}: {mask_path}")
        mask_min = min(mask_min, float(frame.min()))
        mask_max = max(mask_max, float(frame.max()))
    if mask_min < 0.0 or mask_max > 1.0:
        raise RuntimeError(f"HaWoR model mask values must lie in [0, 1]; got [{mask_min}, {mask_max}]: {mask_path}")
    mask_hash = sha256(mask_path)
    report = {
        "schema": "v22_hawor_motion_preparation.v0",
        "status": "ok",
        "video": {"path": str(video_path), "sha256": video_sha256},
        "timeline": {"start_idx": int(start_idx), "end_idx": int(end_idx), "frame_count": int(len(imgfiles))},
        "seq_folder": str(seq_folder),
        "image_dir": str(Path(imgfiles[0]).parent) if imgfiles else str(seq_folder / "extracted_images"),
        ...
        "artifacts": {
            "dynamic_mask": {
                ...
                "value_range": [mask_min, mask_max],
                "source": "hawor_model_masks",
            },
            "frame_chunks": {"path": str(chunks_path), "sha256": sha256(chunks_path)},
        },
        "contract": {
            "droid_consumption": "canonical DROID must use this full-resolution mask artifact for image zeroing and stride-8 track confidence",
            "final_hawor_consumption": "normal export reuses frame_chunks_all.npy and model_masks.npy; it must not instantiate DROID",
        },
    }
    report_path = output_dir / "hawor_motion_preparation.json"
...
def run(args: argparse.Namespace) -> dict:
    ...
    if args.droid_shared_manifest is not None:
        shared_manifest_path = Path(args.droid_shared_manifest).expanduser()
        if not shared_manifest_path.is_absolute():
            shared_manifest_path = (Path.cwd() / shared_manifest_path).resolve()
        if not shared_manifest_path.is_file() and not args.prepare_only:
            raise FileNotFoundError(f"shared DROID manifest is missing: {shared_manifest_path}")
        args.droid_shared_manifest = str(shared_manifest_path)
    elif not args.prepare_only:
        raise RuntimeError("--droid-shared-manifest is required; legacy HaWoR DROID SLAM is disabled")
    ...
    from scripts.scripts_test_video.hawor_video import hawor_infiller, hawor_motion_estimation  # type: ignore
    ...
    preparation_report = write_motion_preparation_report(
        ...
        end_idx=int(end_idx),
        imgfiles=[Path(path) for path in imgfiles],
        img_focal=float(img_focal),
        focal_cache_contract=focal_cache_contract,
    )
    if args.prepare_only:
        preparation_report["export_provenance"] = export_provenance
        preparation_report["status"] = "ok"
        Path(preparation_report["report_path"]).write_text(json.dumps(preparation_report, indent=2), encoding="utf-8")
        print(json.dumps(preparation_report, indent=2))
        return preparation_report
    if shared_manifest_path is None or not shared_manifest_path.is_file():
        raise RuntimeError("shared DROID manifest is required before HaWoR finalization")
    slam_path = Path(seq_folder) / "SLAM" / f"hawor_slam_w_scale_{start_idx}_{end_idx}.npz"
    mask_path = Path(seq_folder) / f"tracks_{start_idx}_{end_idx}" / "model_masks.npy"
    first_image = cv2.imread(str(imgfiles[0]), cv2.IMREAD_COLOR) if imgfiles else None
    if first_image is None:
        raise RuntimeError("HaWoR extracted image list is empty or unreadable")
    image_height, image_width = first_image.shape[:2]
    adapter_report = adapt_droid_to_hawor(
        shared_manifest_path,
        output_path=slam_path,
        imgfiles=[str(path) for path in imgfiles],
        masks_path=mask_path,
        img_focal=float(img_focal),
        img_center=(float(image_width) / 2.0, float(image_height) / 2.0),
        hawor_root=hawor_root,
        expected_frames=len(imgfiles),
        expected_clip_sha256=video_sha256,
        metric_checkpoint=args.metric_depth_weight,
    )
    if not slam_path.exists():
        raise RuntimeError(f"HaWoR adapter output missing: {slam_path}")
    adapter_report_source = Path(str(adapter_report["report_path"]))
    adapter_report_published = args.output_dir / "hawor_slam_adapter_report.json"
    shutil.copy2(adapter_report_source, adapter_report_published)
    adapter_report["published_report_path"] = str(adapter_report_published)
    ...
    np.savez_compressed(
        ...
        hawor_motion_preparation=np.asarray([str(preparation_report.get("report_path", ""))]),
        hawor_motion_preparation_sha256=np.asarray([sha256(Path(str(preparation_report["report_path"])))]) if preparation_report.get("report_path") else np.asarray([""]),
        droid_shared_manifest=np.asarray([str(args.droid_shared_manifest)]),
        droid_shared_manifest_sha256=np.asarray([sha256(Path(args.droid_shared_manifest))]),
        droid_adapter_report=np.asarray([str(adapter_report.get("published_report_path", adapter_report.get("report_path", "")))]),
    ...
    qc = {
        ...
        "focal_cache_contract": focal_cache_contract,
        "hawor_motion_preparation": preparation_report,
        "slam_path": str(slam_path),
        "droid_adapter": adapter_report,
        "droid_shared_manifest": str(args.droid_shared_manifest),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--droid-shared-manifest", type=Path, default=None, help="D4 shared DROID geometry manifest; required because legacy HaWoR DROID SLAM is disabled.")
    parser.add_argument("--prepare-only", action="store_true", help="Prepare HaWoR motion/model masks for the canonical DROID run and stop before adapter/infiller export.")
    parser.add_argument("--metric-depth-weight", type=Path, default=None, help="Optional Metric3D checkpoint used only for the adapter scale estimate.")
