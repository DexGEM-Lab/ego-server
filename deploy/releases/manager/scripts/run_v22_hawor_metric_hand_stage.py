...
from scripts.v22_model_request_helpers import write_hawor_request
...
def build_command(args: argparse.Namespace, run_root: Path, clip: Path, focal: float, clip_hash: str, output_dir: Path, droid_shared_manifest: Path | None) -> tuple[list[str], dict[str, str]]:
    env = os.environ.copy()
    env.update(
        {
            ...
            "EGO_HAWOR_PREPARE_ONLY": "1" if args.prepare_motion_only else "0",
        }
    )
    if args.direct_export:
        ...
        if args.prepare_motion_only:
            cmd.append("--prepare-only")
        else:
            if droid_shared_manifest is None:
                raise RuntimeError("normal HaWoR export requires a shared DROID manifest")
            cmd.extend(["--droid-shared-manifest", str(droid_shared_manifest)])
        if args.metric_depth_weight is not None:
            cmd.extend(["--metric-depth-weight", str(args.metric_depth_weight.expanduser().resolve())])
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    prepare_only = bool(args.prepare_motion_only)
    droid_shared_manifest: Path | None = None
    if not prepare_only:
        droid_shared_manifest = (args.droid_shared_manifest or (run_root / "measurements" / "camera_trajectory" / "droid_full_frame" / "droid_shared_geometry.json")).expanduser().resolve()
        if not droid_shared_manifest.is_file():
            raise FileNotFoundError(f"shared DROID manifest required by HaWoR stage: {droid_shared_manifest}")
    model_request = write_hawor_request(
        run_root,
        output_dir=output_dir,
        calibration_contract=calibration_contract,
        droid_shared_manifest=droid_shared_manifest,
    )
    declared_manifest = Path(str(model_request["droid_shared_manifest"])).expanduser().resolve()
    if not prepare_only:
        if droid_shared_manifest != declared_manifest:
            raise RuntimeError(
                "HaWoR request droid_shared_manifest disagrees with the execution input: "
                f"request={declared_manifest} execution={droid_shared_manifest}"
            )
        droid_shared_manifest = declared_manifest
    request_path = run_root / "requests" / ("hawor_prepare.json" if prepare_only else "hawor.json")
    if prepare_only:
        model_request["stage"] = "D5a_hawor_motion_preparation"
    write_json(request_path, model_request)
    log_path = output_dir / ("run_hawor_motion_preparation.log" if prepare_only else "run_hawor_metric_hand_stage.log")
    cmd, env = build_command(args, run_root, clip, focal, clip_hash, output_dir, droid_shared_manifest)
    ...
    stage: dict[str, Any] = {
        ...
        "droid_shared_manifest": str(droid_shared_manifest) if droid_shared_manifest is not None else None,
        "command": cmd,
        "model_request": str(request_path),
        "model_request_payload": model_request,
        "environment": env_view,
        "claim_scope": "D5 HaWoR metric MANO execution stage; no WiLoR substitution and no hybrid fusion acceptance." if not prepare_only else "D5a HaWoR motion/mask preparation for the canonical DROID run; no hand-state or DROID inference claim.",
        "prepare_only": prepare_only,
        "log": str(log_path),
    }
    stage_path = output_dir / ("v22_hawor_motion_preparation_stage.json" if prepare_only else "v22_hawor_metric_hand_stage.json")
    if args.dry_run:
    ...
    if proc.returncode == 0:
        if prepare_only:
            preparation_path = output_dir / "hawor_motion_preparation.json"
            if not preparation_path.is_file():
                stage["status"] = "failed_missing_motion_preparation"
            else:
                preparation = load_json(preparation_path)
                dynamic_mask = ((preparation.get("artifacts") or {}).get("dynamic_mask") if isinstance(preparation.get("artifacts"), dict) else None)
                chunks = ((preparation.get("artifacts") or {}).get("frame_chunks") if isinstance(preparation.get("artifacts"), dict) else None)
                expected = expected_frame_count(run_root)
                frame_count = ((preparation.get("timeline") or {}).get("frame_count") if isinstance(preparation.get("timeline"), dict) else None)
                checks = [
                    {"check": "preparation_status_ok", "ok": preparation.get("status") == "ok"},
                    {"check": "dynamic_mask_artifact", "ok": isinstance(dynamic_mask, dict) and Path(str(dynamic_mask.get("path", ""))).is_file()},
                    {"check": "frame_chunks_artifact", "ok": isinstance(chunks, dict) and Path(str(chunks.get("path", ""))).is_file()},
                ]
                if expected is not None:
                    checks.append({"check": "frame_count_matches_raw_manifest", "ok": int(frame_count or -1) == int(expected), "expected": expected, "actual": frame_count})
                stage["preparation"] = {"path": str(preparation_path), "payload": preparation, "checks": checks}
                if not all(bool(row["ok"]) for row in checks):
                    stage["status"] = "failed_invalid_motion_preparation"
        else:
    ...
    print(json.dumps(stage, indent=2))
    if stage["status"] != "ok":
        raise RuntimeError(f"HaWoR stage failed; see {stage_path}")
    return stage
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--droid-shared-manifest", type=Path, default=None, help="D4 shared DROID geometry manifest consumed by the HaWoR adapter.")
    parser.add_argument("--prepare-motion-only", action="store_true", help="Prepare HaWoR model masks/frame chunks for canonical DROID, then stop before adapter and MANO export.")
    parser.add_argument("--metric-depth-weight", type=Path, default=None, help="Optional Metric3D checkpoint used by the DROID-to-HaWoR scale adapter.")
