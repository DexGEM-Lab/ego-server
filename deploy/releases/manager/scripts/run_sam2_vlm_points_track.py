...
DEFAULT_SAM2_ROOT = Path(__file__).resolve().parents[1] / "third_party" / "sam2"


def import_sam2_video_predictor(sam2_repo: Path | None):
    root = sam2_repo or DEFAULT_SAM2_ROOT
...
def prompt_coordinate_scale(payload: dict, image_width: int) -> float:
    prompt_width = payload.get("prompt_image_width")
    if prompt_width is None:
        return 1.0
    prompt_width_f = float(prompt_width)
    if prompt_width_f <= 0:
        raise RuntimeError(f"invalid prompt_image_width: {prompt_width}")
    return float(image_width) / prompt_width_f
...
def prompt_box(prompt: dict, scale: float) -> np.ndarray | None:
    raw = prompt.get("bbox_xyxy")
    if not isinstance(raw, list) or len(raw) < 4:
        return None
    return np.asarray([float(v) * scale for v in raw[:4]], dtype=np.float32)
...
def run_sam2(args: argparse.Namespace, frames: list[dict], prompts: dict[int, dict], frame_dir: Path, scale: float) -> dict[int, dict]:
    ...
    build_sam2_video_predictor = import_sam2_video_predictor(args.sam2_repo)
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        for source_idx in prompt_frames:
            ...
            point_coords, point_labels = prompt_points(prompt, scale)
            box = prompt_box(prompt, scale)
...
def run(args: argparse.Namespace) -> dict:
    ...
    cap, info = open_video(args.clip)
    cap.release()
    scale = prompt_coordinate_scale(payload, int(args.sam2_image_width))
    ...
    qc = {
        ...
        "prompt_image_width": payload.get("prompt_image_width"),
        "prompt_to_sam2_scale": float(scale),
