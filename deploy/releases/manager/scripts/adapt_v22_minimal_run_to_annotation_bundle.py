...
def camera_source_from_stage(stage: dict[str, Any]) -> str:
    backend = str(stage.get("camera_backend") or stage.get("backend") or "").strip()
    if backend:
        return "droid_full_frame_video_tracking" if backend == "droid" else f"{backend}_video_tracking"
    return "droid_full_frame_video_tracking"
...
def camera_trajectory_rows(run_root: Path) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    ...
    rows: list[dict[str, Any]] = []
    source = camera_source_from_stage(stage)
    for row in frames:
    ...
    *,
    ...
    if not has_camera:
        ...
        bundle.add_error(
            ...
            "The D4 camera trajectory stage provides a video-derived trajectory estimate; ATE/RPE metric claims still require device VIO/SLAM/IMU, fiducial/mocap, or benchmark GT.",
        )
    if not has_hawor:
        bundle.add_error(
...
def adapt(args: argparse.Namespace) -> Path:
    ...
    render_path_keys = ("v22_overlay", "hand_overlay", "hybrid_hand_overlay", "world_head_hand_3d", "semantic_subtitle")
    render_artifacts = {
