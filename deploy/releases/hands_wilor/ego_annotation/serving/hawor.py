...
def expected_hawor_runtime_config(
    *, batch_cap: int = 4, batch_wait_ms: float = 20.0, max_queued_requests: int = 32, wire_format: str = "multipart",
) -> dict[str, object]:
...
def build_hawor_model_config(
    *,
    ...
    return HaWoRModelConfig(
        ...
        batch_policy=batch_policy or BatchPolicy(max_batch_size=4, batch_wait_timeout_s=0.02, max_queued_requests=32),
    ...
    async def infer_batch(
        ...
        for i, r in enumerate(requests):
            try:
                ...
                # Vertices: the model surface is produced by the MANO query inside
                # forward_step only if requested; here we expose the joints (which
                # include the wrist) and mark vertices as derived-with-uncertainty
                # by re-running the MANO layer is out of scope for the forward-only
                # slice. We expose joints (wrist + 15) as the renderable geometry and
                # carry vertices as a zero-filled placeholder with an explicit note
                # when the backend did not return them. Honest: do not relabel joints
                # as vertices.
                verts_i = np.zeros((HAWOR_CHUNK_LEN, HAWOR_MANO_VERTS, 3), dtype=np.float32)
                if "vertices" in outputs:
                    v = np.asarray(outputs["vertices"])
                    if v.shape == (len(requests), HAWOR_CHUNK_LEN, HAWOR_MANO_VERTS, 3):
                        verts_i = v[i].astype(np.float32)
