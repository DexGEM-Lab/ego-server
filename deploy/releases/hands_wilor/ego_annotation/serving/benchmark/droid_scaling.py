...
from ego_annotation.serving.lifecycle import ClusterLifecycleConfig, committed_serve_http_ports, droid_gpu_group
...
# GPU4 is committed WiLoR production capacity; experiments may use only GPU5/GPU7.
AUTHORIZED_EXPERIMENT_GPU_IDS = frozenset({5, 7})
...
class DroidScalingExperimentPlan:
    ...
    def assert_isolated(self) -> None:
        ...
        if len({replica.gpu_id for replica in self.replicas}) != len(self.replicas):
            raise ExperimentConfigurationError("each DROID replica must own a distinct physical GPU")
        ...
        overlap = set(ports) & PRODUCTION_PORTS
        if overlap:
...
def build_droid_scaling_plan(
    *,
    ...
    if not gpu_ids or len(set(gpu_ids)) != len(gpu_ids):
        raise ExperimentConfigurationError("DROID experiment GPU ids must be non-empty and unique")
    ...
    for index, (gpu_id, gpu_uuid) in enumerate(zip(gpu_ids, gpu_uuids)):
        ports = _experiment_ports(component_port_base + index * 200, worker_port_base + index * 200, serve_port_base + index)
        if any(port <= 0 or port > 65535 for port in ports.all_ports()):
            raise ExperimentConfigurationError("DROID experiment ports must be in 1..65535")
        temp_dir = canonical_temp / experiment_id / f"gpu{gpu_id}"
        ...
        replica_id = f"droid-exp-{experiment_id}-gpu{gpu_id}"
        pythonpath = ":".join(
    ...
    plan = DroidScalingExperimentPlan(
        experiment_id, Path(run_root), release, droid_source_release, corpus_digest, measurement_interval_id, tuple(replicas)
    )
...
def droid_production_health_preflight_command(host: str = "127.0.0.1") -> str:
    """Require every committed production lane before allocating an experiment."""
    checks = []
    for port in committed_serve_http_ports():
        checks.append(
            f"code=$(curl --silent --show-error --max-time 2 --output /dev/null --write-out '%{{http_code}}' "
            f"http://{host}:{port}/-/healthz); rc=$?; test $rc -eq 0 && "
...
def droid_preflight_check_command(replica: ExperimentalDroidReplica) -> str:
    """Atomically preserve live production lanes and reserve one vacant DROID scope."""
    root = shlex.quote(str(DROID_EXPERIMENT_TEMP_ROOT))
    temp_dir = shlex.quote(replica.lifecycle.temp_dir)
    return (
        f"{droid_production_health_preflight_command()} && "
        f"mkdir -p {root} && test -d {root} && test -w {root} && test -x {root} && "
        f"test ! -e {temp_dir} && {preflight_check_command(replica)}"
    )
...
def _is_exact_droid_temp_dir(temp_dir: str | Path) -> bool:
    try:
        ...
        return False
    return len(relative.parts) == 2 and relative.parts[1].startswith("gpu") and relative.parts[1][3:].isdigit()
