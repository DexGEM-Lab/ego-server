...
from ego_annotation.serving.benchmark.droid_scaling import (
    ...
    droid_production_health_preflight_command,
...
def test_droid_plan_retains_short_temp_pyc_exclusion_and_environment_owned_cleanup(tmp_path):
    ...
    assert all(str(port) in preflight for port in (28000, 28001, 28002, 28003, 28004, 28006))
    assert preflight.index("28000") < preflight.index("mkdir -p /tmp/zjheds")
    cleanup = cleanup_verification_command(replica)
    assert "droid_scaling --scoped-status" in cleanup
    assert "ray stop" not in cleanup


def test_droid_production_health_preflight_requires_all_committed_live_lanes(tmp_path):
    fake_curl = tmp_path / "curl"
    fake_curl.write_text(
        "#!/bin/sh\n"
        "case \"$*\" in *:28002/*) printf '%s' \"${DROID_TEST_28002_STATUS:-200}\" ;; *) printf 200 ;; esac\n"
    )
    fake_curl.chmod(0o755)
    command = droid_production_health_preflight_command()
    assert all(str(port) in command for port in (28000, 28001, 28002, 28003, 28004, 28006))
    environment = {**os.environ, "PATH": f"{tmp_path}:{os.environ['PATH']}"}
    assert subprocess.run(["sh", "-c", command], env=environment, check=False).returncode == 0
    unhealthy = {**environment, "DROID_TEST_28002_STATUS": "503"}
    assert subprocess.run(["sh", "-c", command], env=unhealthy, check=False).returncode != 0


def test_two_replicas_have_disjoint_gpu_ports_temp_and_http(tmp_path):
    plan = _plan(tmp_path, gpus=(7, 5))
