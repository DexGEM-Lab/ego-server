...
class DroidAdapter:
    ...
    def admit_frame(self, request: DroidFrameRequest) -> _PreparedFrame | DroidFrameResponse:
        ...
        try:
            ...
            with self._gpu_execution_lock:
                ...
                mask_grid = self._decode_mask(request, state)
    ...
    def _finalize_session_locked(
        ...
        if n_key < 2:
            raise RuntimeError("DROID finalize unresolved: fewer than two keyframes; static trajectory is forbidden")
    ...
    def _decode_mask(self, request: DroidFrameRequest, state: _SessionState) -> Any:
        ...
        arr = self._tensor_resolver(mask.data, mask.shape, mask.dtype)
        arr = np.asarray(arr).astype(np.float32)
        gh, gw = state.image_shape.height // 8, state.image_shape.width // 8
        if arr.shape == (gh, gw):
            return torch.as_tensor(arr, device=self._config.device)
        if arr.shape == (state.image_shape.height, state.image_shape.width):
            # Downsample to the 1/8 grid by mean pooling (matches DROID mask grid).
            import torch.nn.functional as F

            t = torch.as_tensor(arr, device=self._config.device)[None, None]
            t = F.avg_pool2d(t, 8)
            return t[0, 0]
        raise ContractValidationError(
