...
from ego_annotation.serving.hands import HANDS_YOLO_V2_REVISION, HandsAdapter, build_hands_model_config
...
_RUNTIME_ENV_KEYS = (
    "PYTHONPATH", "EGO_SAM2_REPO", "EGO_WILOR_REPO", "EGO_HANDS_GPU", "EGO_HANDS_REPLICA_ID", "EGO_WILOR_REPLICA_ID",
    "EGO_HANDS_REVISION", "EGO_WILOR_REVISION", "EGO_MODEL_CLUSTER_ID", "EGO_MODEL_GCS_ADDRESS", "EGO_MODEL_TEMP_DIR",
...
def _hands_config_from_env() -> Any:
    return build_hands_model_config(
        ...
        model_revision=os.environ.get("EGO_HANDS_REVISION", HANDS_YOLO_V2_REVISION),
...
def build_hands_api() -> FastAPI:
    """Rollback ingress retaining both frozen GPU1 routes."""
    app = FastAPI()
...
def build_hands_only_api() -> FastAPI:
    """Ingress for the GPU1 Hands cluster; WiLoR is deliberately unpublished."""
    app = FastAPI()
...
def build_wilor_only_api() -> FastAPI:
    """Ingress for the GPU4 WiLoR cluster; Hands is deliberately unpublished."""
    app = FastAPI()
...
class _HandsWiLoRHttpHandlers:
    ...
    @serve.batch(max_batch_size=8, batch_wait_timeout_s=0.02, batch_size_fn=canonical_batch_size_fn)
    ...
    @serve.batch(max_batch_size=16, batch_wait_timeout_s=0.02, batch_size_fn=canonical_batch_size_fn)
...
def _hands_arrays(response: HandsDetectResponse) -> dict[str, ArrayPart]:
    ...
    if det.masks is not None:
        arrays["masks"] = (det.masks.data, det.masks.shape, det.masks.dtype)
    return arrays
...
class HandsWiLoRDeployment(_HandsWiLoRHttpHandlers):
    """Rollback deployment retaining the original combined resident replica."""
...
class HandsOnlyDeployment(_HandsWiLoRHttpHandlers):
    """GPU1 deployment that constructs the detector adapter and nothing else."""
...
class WiLoROnlyDeployment(_HandsWiLoRHttpHandlers):
    """GPU4 deployment that constructs the WiLoR adapter and nothing else."""

    def __init__(self) -> None:
        self.wilor = WiLoRAdapter(_wilor_config_from_env())


# Keep the combined application importable as the exact Serve rollback target.
hands_app: Any = HandsWiLoRDeployment.bind()
hands_only_app: Any = HandsOnlyDeployment.bind()
wilor_only_app: Any = WiLoROnlyDeployment.bind()
