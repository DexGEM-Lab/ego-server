"""Build or verify an immutable exact-justified SAM2 source release without model loading."""
...
from ego_annotation.serving.sam2_source import (
    EXACT_GIT_HEAD,
    EXPECTED_CORE_GROUP_DIGEST,
    RECOVERED_AMENDMENT_ID,
    build_sam2_source_release,
    verify_sam2_source_release,
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "build":
        ...
        release = build_sam2_source_release(
        ...
        release = verify_sam2_source_release(
    ...
    print(json.dumps({
        "schema": "ego.sam2-source-release-result.v1", "path": str(release.path),
        "ego_sam2_repo": str(release.path), "source_digest": release.source_digest,
