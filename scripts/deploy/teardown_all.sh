...
log() { printf '[teardown-all] %s\n' "$*"; }
rc=0
for service in manager dispatcher cosmos hawor droid wilor hands unidepth; do
  log "stopping $service"
  "$SCRIPT_DIR/teardown_${service}.sh" || rc=1
done
if (( rc )); then
  log "one or more teardown steps failed" >&2
  exit "$rc"
fi
log "all ego annotation services are stopped"
