...
# Permit dispatcher code to be pinned to an immutable release while retaining
# the vendored release as the normal default.
CODE="${CODE:-$REPO_ROOT/deploy/releases/dispatcher}"
export CODE
assert_release
require_executable "$PY"
require_dir "$CODE"
ensure_session
if window_exists dispatcher; then
  http_ready 28000 && http_ready 28002 && { log "dispatcher already healthy"; exit 0; }
  die "dispatcher window exists but public ports are unhealthy"
fi
# Backends must be healthy before the dispatcher claims the public ports.
http_ready 29000 || die "UniDepth owner backend 29000 is not healthy"
http_ready 28005 || die "UniDepth GPU5 backend 28005 is not healthy"
http_ready 29002 || die "DROID owner backend 29002 is not healthy"
http_ready 28012 || die "DROID backend 28012 is not healthy"
http_ready 28022 || die "DROID backend 28022 is not healthy"
http_ready 28007 || die "DROID backend 28007 is not healthy"
http_ready 28017 || die "DROID backend 28017 is not healthy"
http_ready 28027 || die "DROID backend 28027 is not healthy"
require_free_ports 28000 28002
cmd=$(cat <<'CMD'
set -euo pipefail
export PYTHONPATH="${CODE}"
exec /home/zjh/miniconda3/envs/sharpa_isaaclab/bin/python -m ego_annotation.serving.lane_dispatcher \
  --bind-host 127.0.0.1 --droid-port 28002 --unidepth-port 28000 \
  --db-path /tmp/ego_lane_dispatcher_leases.sqlite3 --unidepth-replica-capacity 8
CMD
)
start_window dispatcher "$CODE" "$cmd"
for port in 28000 28002; do
  deadline=$((SECONDS + 60))
  while (( SECONDS < deadline )); do http_ready "$port" && break; sleep 1; done
  http_ready "$port" || die "dispatcher did not become healthy on $port"
done
log "lane dispatcher deployment complete"
