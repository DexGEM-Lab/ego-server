...
wait_for_hawor_apps() {
  local py=$1 dashboard=$2 http_port=$3 timeout=${4:-600}
  local deadline=$((SECONDS + timeout)) status=""
  log "Waiting for HaWoR and infiller on HTTP $http_port"
  while (( SECONDS < deadline )); do
    status=$($py -m ray.serve.scripts status -a "http://127.0.0.1:${dashboard}" 2>&1 || true)
    if http_ready "$http_port" \
      && grep -A2 -F '  hawor-infer-tracks:' <<<"$status" | grep -Fqx '    status: RUNNING' \
      && grep -A2 -F '  hawor-infiller-fill:' <<<"$status" | grep -Fqx '    status: RUNNING'; then
      log "HaWoR and infiller are healthy"
      return 0
    fi
    sleep 3
  done
  printf '%s\n' "$status" >&2
  die "HaWoR and infiller did not both become RUNNING within ${timeout}s"
}

PY="/home/zjh/miniconda3/envs/ray_serve_hawor/bin/python"
assert_release
require_executable "$PY"
require_dir /vePFS-Mindverse/user/yiwen/user-home/zjh/ego_annotation-master/third_party/HaWoR
ensure_session
if window_exists hawor-gpu3; then wait_for_hawor_apps "$PY" 26604 28003 30; exit 0; fi
require_free_ports 26600 26601 26602 26603 26604 26605 26606 26607 28003 26700 26701 26702 26703 26704 26705 26706 26707 26708 26709 26710 26711 26712 26713 26714 26715 26716 26717 26718 26719 26720 26721 26722 26723 26724 26725 26726 26727 26728 26729 26730 26731
cmd=$(cat <<'CMD'
set -euo pipefail
export CUDA_VISIBLE_DEVICES=3 PYTHONPATH="${RUNTIME}"
export EGO_HAWOR_REPO=/vePFS-Mindverse/user/yiwen/user-home/zjh/ego_annotation-master/third_party/HaWoR
export EGO_HAWOR_CHECKPOINT=/vePFS-Mindverse/user/yiwen/user-home/zjh/ego-annation-checkpoints/hawor/hawor.ckpt
export EGO_HAWOR_INFILLER_CHECKPOINT=/vePFS-Mindverse/user/yiwen/user-home/zjh/ego-annation-checkpoints/hawor/infiller.pt
export EGO_HAWOR_GPU=3 RAY_DEDUP_LOGS=0
"/home/zjh/miniconda3/envs/ray_serve_hawor/bin/python" -m ray.scripts.scripts start --head \
  --port=26600 --dashboard-port=26604 \
  --object-manager-port=26601 --node-manager-port=26602 \
  --ray-client-server-port=26603 --dashboard-agent-listen-port=26605 \
  --dashboard-agent-grpc-port=26606 --metrics-export-port=26607 \
  --worker-port-list=26700,26701,26702,26703,26704,26705,26706,26707,26708,26709,26710,26711,26712,26713,26714,26715,26716,26717,26718,26719,26720,26721,26722,26723,26724,26725,26726,26727,26728,26729,26730,26731 --num-gpus=1 --num-cpus=4 \
  --temp-dir=/tmp/ray-ego-serve-gpu3 --include-dashboard=true
"/home/zjh/miniconda3/envs/ray_serve_hawor/bin/python" -m scripts.serve_group_driver --gpu-id 3 --address 127.0.0.1:26600 --port 28003
exec sleep infinity
CMD
)
start_window hawor-gpu3 "$RUNTIME" "$cmd"
wait_for_hawor_apps "$PY" 26604 28003 900
log "HaWoR deployment complete"
