...
class FakeVideo:
    ...
    def __init__(self) -> None:
        ...
        self.tstamp = np.zeros(64, dtype=np.float64)
...
class FakeCpuOffloader:
    """Host-only lazy offload controller with deterministic materialization evidence."""
    ...
    def initialize(self, video: Any) -> _CpuOffloadState:
        self.initialized.append(id(video))
        return _CpuOffloadState(cpu_tensors={}, capacity=64, cpu_capacity_bytes=4_096)

    def activate(self, offload: _CpuOffloadState, video: Any, required_slots: int) -> None:
        self.activated.append((id(video), required_slots))
        if required_slots > offload.materialized_slots:
            self.materialized.append((id(video), required_slots))
            offload.materialized_slots = required_slots
            offload.cpu_resident_bytes = required_slots * 64
        offload.active_slots = required_slots
        offload.gpu_resident_bytes = 512
        offload.gpu_peak_resident_bytes = max(offload.gpu_peak_resident_bytes, 512)
        offload.last_transfer_ms = 1.25

    def release(self, offload: _CpuOffloadState, video: Any) -> None:
        self.released.append(id(video))
        offload.active_slots = 0
        offload.gpu_resident_bytes = 0
        offload.last_transfer_ms += 2.75
...
def make_adapter(
    ...
    cpu_offload_controller_factory: Any = None,
    ...
    adapter = DroidAdapter(
        ...
        **({"cpu_offload_controller_factory": cpu_offload_controller_factory}
           if cpu_offload_controller_factory is not None else {}),
    )
...
def test_lazy_cpu_offload_create_s24_has_no_materialized_buffer() -> None:
    """High session count reserves lazy index-space without a serial CPU copy."""
    loads: list[int] = []
    controller = FakeCpuOffloader("cpu")
    adapter, _ = make_adapter(
        loads,
        config=make_config(cpu_offload=True, max_sessions=24),
        cpu_offload_controller_factory=lambda _device: controller,
    )

    started = time.monotonic()
    responses = [adapter.create_session(make_create_request(f"create-{index}")) for index in range(24)]
    elapsed = time.monotonic() - started

    assert all(response.session_id is not None and response.error is None for response in responses)
    assert elapsed < 1.0
    assert len(controller.initialized) == 24
    assert controller.materialized == []
    assert controller.activated == []
    for response in responses:
        assert response.session_id is not None
        assert adapter.session_allocator_memory(response.session_id) == {
            "cpu_offload": True,
            "cpu_resident_bytes": 0,
            "cpu_capacity_bytes": 4_096,
            "cpu_materialized_slots": 0,
            "gpu_resident_bytes": 0,
            "gpu_peak_resident_bytes": 0,
        }


def test_depth_video_cpu_offloader_initializes_only_touched_prefix_defaults() -> None:
    """Lazy CPU storage reconstructs fresh DepthVideo values before first BA use."""
    import torch
    ...
    video = Video()
    video.tstamp = torch.zeros(4)
    video.images = torch.zeros(4, 3, 2, 2, dtype=torch.uint8)
    video.dirty = torch.zeros(4, dtype=torch.bool)
    video.red = torch.zeros(4, dtype=torch.bool)
    video.poses = torch.zeros(4, 7)
    video.poses[:, 6] = 1.0
    video.disps = torch.ones(4, 1, 1)
    video.disps_sens = torch.zeros(4, 1, 1)
    video.disps_up = torch.zeros(4, 2, 2)
    video.intrinsics = torch.zeros(4, 4)
    video.masks = torch.zeros(4, 1, 1)
    video.fmaps = torch.zeros(4, 1, 128, 1, 1, dtype=torch.float16)
    video.nets = torch.zeros(4, 128, 1, 1, dtype=torch.float16)
    video.inps = torch.zeros(4, 128, 1, 1, dtype=torch.float16)

    offloader = _DepthVideoCpuOffloader("cpu")
    offload = offloader.initialize(video)
    assert set(offload.cpu_tensors) == set(_DEPTH_VIDEO_TENSOR_FIELDS)
    assert offload.cpu_resident_bytes == 0
    assert offload.cpu_capacity_bytes == sum(
        _DepthVideoCpuOffloader._tensor_bytes(tensor) for tensor in offload.cpu_tensors.values()
    )
    assert offload.materialized_slots == 0

    offloader.activate(offload, video, 1)
    assert offload.materialized_slots == 1
    assert video.tstamp.shape[0] == 1
    assert torch.equal(video.disps, torch.ones_like(video.disps))
    assert torch.equal(video.poses[..., :6], torch.zeros_like(video.poses[..., :6]))
    assert torch.equal(video.poses[..., 6], torch.ones_like(video.poses[..., 6]))
    video.tstamp[0] = 12.5
    offloader.release(offload, video)

    offloader.activate(offload, video, 3)
    assert offload.materialized_slots == 3
    assert video.tstamp[0].item() == 12.5
    assert torch.equal(video.disps, torch.ones_like(video.disps))
    assert torch.equal(video.poses[..., :6], torch.zeros_like(video.poses[..., :6]))
    assert torch.equal(video.poses[..., 6], torch.ones_like(video.poses[..., 6]))
    offloader.release(offload, video)
...
def test_cpu_offload_fake_backend_preserves_identical_finite_terminal_state() -> None:
    """Placement-only offload must not alter the fake backend's terminal poses."""
    def pose_continuation(_adapter: Any, state: Any, item: Any, _gmap: Any) -> tuple[int, bool]:
        index = state.video.counter.value
        state.video.poses[index] = np.array([
            item.rgb_hwc_uint8[0, 0, 0], item.request.source_timestamp_s, 0, 0, 0, 0, 1,
        ], dtype=np.float64)
        state.video.counter.value += 1
        state.frames_pushed += 1
        return 0, True

    def terminal(adapter: DroidAdapter, sid: str) -> np.ndarray:
        state = adapter._sessions[sid]
        return state.video.poses[:state.video.counter.value].copy()

    resident_loads: list[int] = []
    resident, _ = make_adapter(resident_loads, continuation_fn=pose_continuation)
    offload_loads: list[int] = []
    controller = FakeCpuOffloader("cpu")
    offloaded, _ = make_adapter(
        offload_loads,
        config=make_config(cpu_offload=True),
        continuation_fn=pose_continuation,
        cpu_offload_controller_factory=lambda _device: controller,
    )
    resident_sid = resident.create_session(make_create_request("resident")).session_id
    offloaded_sid = offloaded.create_session(make_create_request("offloaded")).session_id
    for frame_id, ts in (("f0", 0.0), ("f1", 0.033)):
        assert asyncio.run(resident.push_frame_batch([
            resident.admit_frame(make_frame_request(resident_sid, frame_id, ts=ts)),
        ]))[0].error is None
        assert asyncio.run(offloaded.push_frame_batch([
            offloaded.admit_frame(make_frame_request(offloaded_sid, frame_id, ts=ts)),
        ]))[0].error is None

    np.testing.assert_array_equal(terminal(offloaded, offloaded_sid), terminal(resident, resident_sid))
    assert np.isfinite(terminal(offloaded, offloaded_sid)).all()


def test_cpu_offload_records_transfer_separately_and_releases_active_window() -> None:
    loads: list[int] = []
    controller = FakeCpuOffloader("cpu")
    adapter, _ = make_adapter(
        loads,
    ...
    sid = adapter.create_session(make_create_request()).session_id
    first = asyncio.run(adapter.push_frame_batch([
        adapter.admit_frame(make_frame_request(sid, "f0")),
    ]))[0]
    response = asyncio.run(adapter.push_frame_batch([
        adapter.admit_frame(make_frame_request(sid, "f1", ts=0.033)),
    ]))[0]
    assert first.status is not None and response.status is not None
    assert first.status.trace.phase_timing.cpu_offload_transfer_ms == 4.0
    assert controller.materialized == [
        (id(adapter._sessions[sid].video), 2),
        (id(adapter._sessions[sid].video), 3),
    ]
    phase = response.status.trace.phase_timing
    assert phase.cpu_offload_transfer_ms == 4.0
    assert DroidBatchTrace.from_wire(response.status.trace.to_wire()).phase_timing.cpu_offload_transfer_ms == 4.0
    assert phase.fnet_s is not None and phase.fnet_s >= 0.0
    # Transfer is outside fnet/BA named stage spans and visible as its own field.
    assert phase.frontend_ba_s is None
    assert len(controller.activated) == len(controller.released) == 2
    assert all(video_id == id(adapter._sessions[sid].video) for video_id in controller.released)
    accounting = adapter.session_allocator_memory(sid)
    assert accounting == {
        "cpu_offload": True,
        "cpu_resident_bytes": 192,
        "cpu_capacity_bytes": 4_096,
        "cpu_materialized_slots": 3,
        "gpu_resident_bytes": 0,
        "gpu_peak_resident_bytes": 512,
    }


def test_cpu_offload_default_off_preserves_resident_path_and_accounting() -> None:
    loads: list[int] = []

    def forbidden_controller(_device: str) -> FakeCpuOffloader:
        raise AssertionError("default resident path must not construct an offload controller")

    adapter, _ = make_adapter(loads, cpu_offload_controller_factory=forbidden_controller)
    sid = adapter.create_session(make_create_request()).session_id
    response = asyncio.run(adapter.push_frame_batch([
        adapter.admit_frame(make_frame_request(sid, "f0")),
    ]))[0]
    assert adapter.config.cpu_offload is False
    assert response.status is not None
    assert response.status.trace.phase_timing.cpu_offload_transfer_ms is None
    accounting = adapter.session_allocator_memory(sid)
    assert accounting["cpu_offload"] is False
    assert accounting["gpu_resident_bytes"] == 0  # fake DepthVideo stores numpy on CPU
...
def test_dynamic_growth_preserves_predictive_seed_and_damping_prefix() -> None:
    """Growth copies counter+1, including the frontend's predictive seed."""
    import types

    loads: list[int] = []
    adapter, _ = make_adapter(loads, config=make_config(max_buffer_slots=1024))
    create = make_create_request("grow-seed")
    sid = adapter.create_session(create).session_id
    state = adapter._sessions[sid]
    state.video.masks = np.zeros((64, 2, 3), dtype=np.float32)
    state.frontend.graph = types.SimpleNamespace(damping=np.full((64, 2, 3), 5.0, dtype=np.float32))
    state.video.counter.value = 2
    state.video.tstamp[2] = 12.5
    state.video.poses[2] = np.array([9, 8, 7, 6, 5, 4, 3], dtype=np.float64)
    state.video.disps[2] = 7.0
    adapter._ensure_video_capacity(state, 300)
    assert state.effective_capacity >= 300
    assert state.video.tstamp[2] == 12.5
    np.testing.assert_array_equal(state.video.poses[2], [9, 8, 7, 6, 5, 4, 3])
    assert np.all(state.video.disps[2] == 7.0)
    np.testing.assert_array_equal(state.frontend.graph.damping[:3], 5.0)
    assert state.video.tstamp[3] == 0.0
    assert np.all(state.video.disps[3] == 1.0)
    np.testing.assert_array_equal(state.video.poses[3, :6], 0.0)
    assert state.video.poses[3, 6] == 1.0
    assert state.video.masks.shape[0] == state.video_capacity


def test_long_session_grows_past_initial_buffer_and_finalizes_finite_trajectory() -> None:
    """The fake session harness exercises 720 accepted frames without GPU/DROID."""
    loads: list[int] = []
    adapter, _ = make_adapter(
        loads,
        config=make_config(max_buffer_slots=1024),
        continuation_fn=None,
    )
    create = make_create_request("long")
    create = DroidCreateSessionRequest(
        ownership=create.ownership, camera=create.camera, image_shape=create.image_shape,
        model_revision=create.model_revision, options=DroidSessionOptions(buffer=256),
    )
    sid = adapter.create_session(create).session_id

    def every_frame(_adapter: Any, state: Any, item: Any, _gmap: Any) -> tuple[int, bool]:
        _record_exact_dense_source_frame(state, item)
        index = state.video.counter.value
        state.video.poses[index] = np.array([float(index), 0, 0, 0, 0, 0, 1], dtype=np.float64)
        state.video.counter.value += 1
        state.frames_pushed += 1
        return 0, True

    adapter._continuation_fn = every_frame
    for index in range(720):
        response = asyncio.run(adapter.push_frame_batch([
            adapter.admit_frame(make_frame_request(sid, f"long-{index}", ts=index / 30.0, pixels=index % 251)),
        ]))[0]
        assert response.error is None
    state = adapter._sessions[sid]
    assert state.frames_pushed == 720
    assert state.effective_capacity > 256
    assert len(state.dense_source) == len(state.frame_request_owners) == 720

    def fake_finalize(state_: Any, _batch: str, _started: float, ownership: Ownership) -> CameraState:
        assert state_.effective_capacity >= int(state_.video.counter.value) + 16
        return make_camera_state(ownership, state_.session_id)

    adapter._finalize_session = fake_finalize  # type: ignore[method-assign]
    finalized = asyncio.run(adapter.finalize(DroidFinalizeRequest(
        ownership=Ownership("long-finalize", "job", "item", "droid.finalize", "src"),
        session_id=sid,
        model_revision=REVISION,
    )))
    assert finalized.error is None and finalized.camera_state is not None
    world = np.frombuffer(finalized.camera_state.T_world_camera.data, dtype=np.float64).reshape(finalized.camera_state.T_world_camera.shape)
    assert np.isfinite(world).all()


def test_damping_grows_independently_when_video_buffer_already_large() -> None:
    """The frontend graph damping is slot-indexed and can be undersized from a
    transient GPU prefix during warmup even when the DepthVideo buffer is already
    large (B1024 path where the buffer never triggers growth). _ensure_video_capacity
    must grow damping to cover the required slots independently of buffer growth,
    or BA indexes damping[counter] past its transient-init size.
    """
    import types

    loads: list[int] = []
    adapter, _ = make_adapter(loads, config=make_config(max_buffer_slots=8192))
    create = make_create_request("damping-b1024")
    sid = adapter.create_session(create).session_id
    state = adapter._sessions[sid]
    # B1024 buffer is already large; simulate the transient-GPU-prefix damping
    # (size 8 from warmup=8, even though the video buffer is 1024).
    state.frontend.graph = types.SimpleNamespace(damping=np.full((8, 2, 3), 5.0, dtype=np.float32))
    state.video.counter.value = 7
    # Required slots counter+2 = 9, which exceeds the undersized damping (8)
    # but NOT the video buffer (1024). Without the fix, _ensure_video_capacity
    # returns early (buffer already large) and damping stays at size 8.
    adapter._ensure_video_capacity(state, 9)
    assert state.frontend.graph.damping.shape[0] >= 9, (
        "damping must grow to cover required slots even when the video buffer is already large"
    )
    # The existing prefix (indices 0..7) must be preserved.
    np.testing.assert_array_equal(state.frontend.graph.damping[:8], 5.0)
...
def test_frame_capacity_bounds_dense_retention_and_retire_releases_it() -> None:
    ...
    # The request buffer is an initial capacity, so the second frame is accepted
    # after a service-level grow rather than rejected at buffer=1.
    assert asyncio.run(adapter.push_frame_batch([adapter.admit_frame(make_frame_request(sid, "f1"))]))[0].error is None
    assert state.effective_capacity >= 3
...
def test_gpu_resident_concurrent_ba_scoped_and_diagnostics() -> None:
    """GPU-resident concurrent BA is scoped: the permit applies only when
    cpu_offload=False; the diagnostics attest both the configured and effective
    concurrency. Two GPU-resident sessions still share one fused fnet, and their
    continuations may overlap without the offload H2D hazard.
    """
    loads: list[int] = []
    config = make_config(max_concurrent_ba=2, cpu_offload=False)
    adapter, backend = make_adapter(loads, config=config)
    diag = adapter.config.runtime_config_wire()
    assert diag["max_concurrent_ba"] == 2
    assert diag["effective_max_concurrent_ba"] == 2
    sid_a = adapter.create_session(make_create_request("a")).session_id
    sid_b = adapter.create_session(make_create_request("b")).session_id
    prepared_a = adapter.admit_frame(make_frame_request(sid_a, "fa", ts=0.0))
    prepared_b = adapter.admit_frame(make_frame_request(sid_b, "fb", ts=0.1))
    ...
    def overlapping_continuation(owner: Any, state: Any, item: Any, fmap: Any) -> tuple[int, bool]:
        barrier.wait(timeout=2.0)
        return fake_continuation(owner, state, item, fmap)

    adapter._continuation_fn = overlapping_continuation
    responses = asyncio.run(adapter.push_frame_batch([prepared_a, prepared_b]))
    assert len(responses) == 2
    assert backend.fnet_calls == [2]
    assert adapter.concurrency_diagnostics() == {"peak_simultaneous_forwards": 2}
    for r in responses:
        assert r.status is not None


def test_offload_serializes_even_with_concurrent_ba_configured() -> None:
    """CPU offload always serializes: the diagnostics report effective concurrency 1
    even when max_concurrent_ba > 1, because the offload H2D transfer + tensor
    lifetime interaction under concurrency is unsafe (off8/off9 regression).
    """
    loads: list[int] = []
    config = make_config(max_concurrent_ba=4, cpu_offload=True)
    adapter, _backend = make_adapter(loads, config=config)
    diag = adapter.config.runtime_config_wire()
    assert diag["max_concurrent_ba"] == 4
    assert diag["effective_max_concurrent_ba"] == 1
...
class ReusingFeatureBackend(FakeBackendModules):
    """Makes cache ownership observable: every fnet call reuses one output buffer."""
    ...
    def fnet(self, images: Any) -> Any:
        ...
        gh, gw = int(arr.shape[3] // 8), int(arr.shape[4] // 8)
        if self._buffer is None or self._buffer.shape[0] < batch:
            self._buffer = np.empty((batch, 1, 128, gh, gw), dtype=np.float32)
        markers = list(range(self._next_marker, self._next_marker + batch))
        self._next_marker += batch
        for index, marker in enumerate(markers):
            self._buffer[index].fill(float(marker))
        self.fnet_calls.append(batch)
        return self._buffer[:batch]


def test_single_push_owns_cached_features_and_consumes_them_in_frame_order() -> None:
    """A reused fnet output buffer cannot overwrite an earlier cached frame."""
    backend = ReusingFeatureBackend()
    config = make_config(max_sessions=1, max_fnet_batch_size=2)
    seen: list[tuple[str, float, int]] = []

    def continuation(owner: Any, state: Any, item: Any, feature: Any) -> tuple[int, bool]:
        feature_array = np.asarray(feature)
        seen.append((item.request.frame_id, float(feature_array.mean()), id(feature)))
        return fake_continuation(owner, state, item, feature)
    ...
    first = adapter.create_session(make_create_request("single-push"))
    assert first.session_id is not None
    assert adapter.create_session(make_create_request("second-session")).error.code is ErrorCode.BACKPRESSURE
    frames = [
        make_frame_request(first.session_id, f"frame-{index}", ts=index / 30.0, pixels=80 + index)
        for index in range(4)
    ]

    responses = asyncio.run(adapter.infer_frames(frames))

    assert all(response.error is None for response in responses)
    assert backend.fnet_calls == [2, 2]
    assert [(frame_id, marker) for frame_id, marker, _ in seen] == [
        ("frame-0", 0.0), ("frame-1", 1.0), ("frame-2", 2.0), ("frame-3", 3.0),
    ]
    assert len({identity for _, _, identity in seen}) == 4
    assert adapter._sessions[first.session_id].in_flight is False


def test_single_push_rejects_more_than_256_frames_before_admission() -> None:
    loads: list[int] = []
    adapter, _ = make_adapter(loads, config=make_config(max_sessions=1, max_fnet_batch_size=256))
    created = adapter.create_session(make_create_request("too-many"))
    assert created.session_id is not None
    frames = [make_frame_request(created.session_id, f"frame-{index}", ts=float(index)) for index in range(257)]

    with pytest.raises(ContractValidationError, match="at most 256"):
        asyncio.run(adapter.infer_frames(frames))
    assert adapter._sessions[created.session_id].result_journal == {}
