...
def test_verifier_canonicalizes_manifest_path_order(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    candidate, group = _candidate(tmp_path, monkeypatch)
    # pathlib orders a child of `order_case/` before `order_case.py`, while
    # canonical manifest paths order the file before the child. The verifier
    # must compare canonical orders without changing the published digest.
    (candidate / "order_case.py").write_text("sibling file")
    nested = candidate / "order_case" / "nested.txt"
    nested.parent.mkdir()
    nested.write_text("nested file")

    release = source.build_hawor_source_release(
        candidate, tmp_path / "releases", origin_evidence={"path_order": "fixture"},
        expected_core_group_digest=group,
    )

    assert source.verify_hawor_source_release(
        release.path, expected_digest=release.source_digest, expected_core_group_digest=release.core_group_digest,
    ) == release
