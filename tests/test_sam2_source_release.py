"""CPU-only adversarial tests for immutable SAM2 source identity."""
...
import ego_annotation.serving.sam2_source as source
...
def _candidate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, str]:
    ...
    # Dotted config directories expose pathlib's non-canonical path ordering;
    # release verification must use explicit POSIX relative-path order.
    for relative in ("sam2/configs/sam2/sam2_hiera_l.yaml", "sam2/configs/sam2.1/sam2.1_hiera_l.yaml"):
        path = root / relative
        path.parent.mkdir(parents=True, exist_ok=True)
...
def _build(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    return source.build_sam2_source_release(
...
def test_verifier_rejects_source_mutation_symlink_and_bytecode(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(source.SAM2SourceVerificationError, match="manifest does not match"):
        source.verify_sam2_source_release(release.path, expected_core_group_digest=release.core_group_digest)
    ...
    with pytest.raises(source.SAM2SourceVerificationError, match="symlink"):
        source.verify_sam2_source_release(clean.path, expected_core_group_digest=clean.core_group_digest)
    ...
    with pytest.raises(source.SAM2SourceVerificationError, match="executable bytecode"):
        source.verify_sam2_source_release(fresh.path, expected_core_group_digest=fresh.core_group_digest)
...
def test_builder_rejects_candidate_symlink_and_wrong_core_hashes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(source.SAM2SourceVerificationError, match="symlink"):
        source.build_sam2_source_release(
            candidate, tmp_path / "releases", origin_evidence={"healthy_worker_pythonpath_report": "fixture"},
            expected_core_group_digest=group,
        )
    ...
    with pytest.raises(source.SAM2SourceVerificationError, match="adapter module hashes differ"):
        source.build_sam2_source_release(
