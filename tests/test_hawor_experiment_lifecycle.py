...
from ego_annotation.serving.infiller import build_infiller_model_config
...
def test_hawor_pair_adapter_defaults_match_deployment_batch_caps():
    hawor = build_hawor_model_config(checkpoint="hawor.ckpt", model_revision="hawor-v1", device="cpu")
    infiller = build_infiller_model_config(checkpoint="infiller.pt", model_revision="hawor-infiller-v1", device="cpu")

    assert (hawor.batch_policy.max_batch_size, hawor.batch_policy.batch_wait_timeout_s, hawor.batch_policy.max_queued_requests) == (8, 0.1, 64)
    assert (infiller.batch_policy.max_batch_size, infiller.batch_policy.batch_wait_timeout_s, infiller.batch_policy.max_queued_requests) == (4, 0.1, 64)
